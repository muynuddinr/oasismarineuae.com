...
export async function GET(request: NextRequest) {
  // 🔒 AUTHENTICATION CHECK - Only admins can view contact details
  const authError = await requireAdminAuth(request);
  if (authError) {
    logAdminActionFromRequest(request, 'GET_CONTACT_SUBMISSIONS_UNAUTHORIZED', false, null, 'Unauthorized attempt');
    return authError;
  }
  ...
  try {
    ...
    // 📝 LOG SUCCESS
    logAdminActionFromRequest(request, 'GET_CONTACT_SUBMISSIONS_SUCCESS', true, { count: formattedSubmissions.length });
