...
export async function POST(request: NextRequest) {
  try {
    ...
    // In production (Vercel), we can't create files, so just return success
    if (process.env.NODE_ENV === 'production') {
      return NextResponse.json({ 
        message: 'Page creation simulated in production', 
        path: cleanPath,
        note: 'File creation is not supported in serverless environments'
      });
    }
